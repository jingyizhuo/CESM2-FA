...
!slarson start
! variables with “save” are retained throughout model
! integration 
! toread is used as a counter so that the input files
! are only read the first time master_task is invoked.
! when under the master_task, the wind stress variable
! is gathered as a global variable (not split between
! various processors) so this is when you want to 
! add your perturbations or overwrite the variables
! with your own

   integer, parameter :: dbl_kind = selected_real_kind(13)
   integer (kind=int_kind) :: ii,nu0,nu1,nu2,k
   real(kind=dbl_kind), save :: tx_c(nx_global,ny_global,12)
   real(kind=dbl_kind), save :: ty_c(nx_global,ny_global,12)
   real(kind=dbl_kind), save :: smfmask_c(nx_global,ny_global)
!   real(kind=dbl_kind), save :: masklon_c(nx_global,ny_global)
   integer (kind=int_kind), save :: toread
   data toread /0/
   real(r8) :: tx_g(nx_global,ny_global)
   real(r8) :: ty_g(nx_global,ny_global)
   real(r8) :: smfmask_g(nx_global,ny_global)
   real(r8), dimension(nx_block,ny_block,max_blocks_clinic) :: &
              tx_l,ty_l,smfmask_l,WORK_1,WORK_2
   ...
   WORK_2 = c0
       ...
       if (my_task == master_task) then
        ...
        if ( toread == 0 ) then
         tx_c = c0 ; ty_c = c0 ; smfmask_c = c0 ;
        ! ADD MASK WLGL
        call get_unit(nu0)
        open(nu0,file='mask.POPd.dat',form='unformatted',access='stream',convert='big_endian')
           read(nu0)smfmask_c
        close(nu0)
        call release_unit(nu0)

!        call get_unit(nu4)
!        open(nu4,file='masklon.dat_r8',form='unformatted',access='stream',convert='big_endian')
!           read(nu4)masklon_c
!        close(nu4)
!        call release_unit(nu4)

        call get_unit(nu1)
        open(nu1,file='tx.POPd.dat',form='unformatted',access='stream',convert='big_endian')
          read(nu1)tx_c
        close(nu1)
        call release_unit(nu1)

        call get_unit(nu2)
        open(nu2,file='ty.POPd.dat',form='unformatted',access='stream',convert='big_endian')
          read(nu2)ty_c
        close(nu2)
        call release_unit(nu2)
...
!           print *, "GILI mask check global",smfmask_g(61:74,75)
!           print *, "GILI mask check global inv",smfmask_g(75,61:74)
!           print *, "GILI mask check global all",smfmask_g
!           print *, "GILI check masklon(:,1)",masklon_c(:,1)
!           print *, "GILI check masklon(2,:)",masklon_c(2,:)
!           print *, "GILI imonth",imonth
!           print *, "GILI tx_c check ",tx_c(61:74,75,imonth)  

       endif


           call scatter_global(tx_l,tx_g, master_task, distrb_clinic, &
                      field_loc_NEcorner, field_type_scalar)
           call scatter_global(ty_l,ty_g, master_task, distrb_clinic, &
                      field_loc_NEcorner, field_type_scalar)
           call scatter_global(smfmask_l,smfmask_g, master_task, distrb_clinic, &
                      field_loc_NEcorner, field_type_scalar)

! GL TEST CODE
!     tx_l(:,:,:)=c0
!     ty_l(:,:,:)=c0
!           print *, "GILI Dims:",nx_block,nx_global,ny_block,ny_global
!           print *, "GILI nblocks_clinic",nblocks_clinic
!           print *, "GILI mask check",smfmask_l(:,:,:)
!           print *, "GILI tx check ",tx_l(:,:,:)


!WORK1 and WORK2 are the original taux and tauy global
!variables. they are intent(in) meaning they can only be
!read in and used but cannot be changed. Here I define
!my own WORK_1 and WORK_2 with the input data




        WORK_1(:,:,:)=WORK1(:,:,:) + &
              smfmask_l(:,:,:)*(tx_l(:,:,:)/momentum_factor)
        WORK_2(:,:,:)=WORK2(:,:,:) + &
              smfmask_l(:,:,:)*(ty_l(:,:,:)/momentum_factor)

!For my purposes I do not use the original WORK1 AND 
!WORK2 and use WORK_1 AND WORK_2 instead (see below). 
!If you want to add your perturbation (WORK_1 and WORK_2)
!to the freely evolving wind stress, Add WORK1 and WORK2 
!to WORK_1 and WOR K_2. 
...
!-----------------------------------------------------------------------
   ...
   SMFT(:,:,1,:) = (WORK_1(:,:,:)*cos(ANGLET(:,:,:)) +  &
                    WORK_2(:,:,:)*sin(ANGLET(:,:,:)))*  &
                    RCALCT(:,:,:)*momentum_factor
   SMFT(:,:,2,:) = (WORK_2(:,:,:)*cos(ANGLET(:,:,:)) -  &
                    WORK_1(:,:,:)*sin(ANGLET(:,:,:)))*  &
                    RCALCT(:,:,:)*momentum_factor
